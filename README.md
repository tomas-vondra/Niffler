...
- **The four registries** — `niffler/strategies/registry.py`,
  `niffler/exporters/registry.py`, `niffler/data/downloaders/registry.py` and
  `niffler/risk/registry.py` — are the only
  name→class maps in the project. Every `--strategy`, `--exporters`, `--source` and
  `--risk-manager` choice
